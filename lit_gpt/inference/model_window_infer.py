...
class GPTwindow(nn.Module):
    ...
    def forward(
        ...
        # first = torch.randn_like(x[:, [0], :]) * math.sqrt(2.0 / 5 / self.config.n_embd)
        # x = torch.cat([first, x[:, 1:, :]], dim=1)
        # print(x.shape)
...
def eager_scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None, window_size=-1) -> torch.Tensor:
    ...
    if window_size != -1:
        ...
        attn_bias.masked_fill_(context_mask.bool(), float("-inf"))
    # print((attn_bias == 0).to(torch.float).sum(dim=-1))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # v = torch.cat([v_bias, v[:, 1:]], dim=1)
        y, attns = self.scaled_dot_product_attention(q, k, v, mask=mask, output_attention=output_attention)
    ...
    def scaled_dot_product_attention(
        ...
        #     return flash_attn_func(q, k, v, dropout_p=self.config.attn_pdrop, softmax_scale=scale, causal=True, window_size=(self.config.window_size-1, 0)), None
        ...
        if output_attention:
            ...
            q, k, v, attn_mask=mask, dropout_p=self.config.attn_pdrop, scale=scale, is_causal=mask is None, window_size=self.config.window_size
            )
        else:

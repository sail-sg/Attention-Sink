    ...
    def forward(
        ...
        if not use_kv_cache:
            for block in self.transformer.h:
                x, *_, attns, x_prenorm = block(x, (cos, sin), max_seq_length, output_attention=output_attention)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        # attention
        h, new_kv_cache, attns = self.attn(x, rope, max_seq_length, mask, input_pos, kv_cache, output_attention=output_attention)
        x = x + h
        x = self.norm_1(x)       # post-norm
        ffn = self.mlp(x)
        if self.config.resid_pdrop:
            ffn = nn.functional.dropout(ffn, p=self.config.resid_pdrop, training=self.training)
        x_prenorm = x + ffn
        x = self.norm_2(x_prenorm)       # post-norm
        return x, new_kv_cache, attns, x_prenorm

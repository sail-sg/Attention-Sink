...
def eager_scaled_dot_product_attention(query, key, value, sim, attention_normalize, attention_bias, dropout_p=0.0, scale=None) -> torch.Tensor:
    ##########
    ...
    if sim == "sigmoid":
        ...
        attn_weight = nn.functional.elu(attn_weight) + 1.0
    ...
    attn_weight_normalized = attn_weight / (attn_weight.sum(dim=-1, keepdims=True) + 1e-8)
    # print(attn_weight_normalized)
    # if learnable_reweights is not None:
    attn_weight = torch.dropout(attn_weight, dropout_p, train=True)
    return attn_weight @ value, attn_weight_normalized
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        y, attns = self.scaled_dot_product_attention(q, k, v, sim=self.sim, attention_normalize=self.attention_normalize, attention_bias=attention_bias, output_attention=output_attention)  # (B, H, T, C) 
    ...
    def scaled_dot_product_attention(
        self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sim: str = 'sigmoid', attention_normalize: bool = True, attention_bias: Optional[torch.Tensor] = None, output_attention: Optional[bool] = False,
    ):
        ...
        if output_attention:
            y, attns = eager_scaled_dot_product_attention(
                q, k, v, sim=sim, attention_normalize=attention_normalize, attention_bias=attention_bias, dropout_p=self.config.attn_pdrop, scale=scale,
            )
        else:

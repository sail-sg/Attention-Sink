    ...
    def forward(
        ...
        if not use_kv_cache:
            for block in self.transformer.h:
                x, *_, attns= block(x, (cos, sin), max_seq_length, output_attention=output_attention)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        n_1 = self.norm_1(x)
        h, new_kv_cache, attns = self.attn(n_1, rope, max_seq_length, mask, input_pos, kv_cache, output_attention=output_attention)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        if config.k_bias:
            if config.head_bias:
                self.k_bias = nn.Parameter(torch.zeros(config.n_head, config.head_size))
            else:
                self.k_bias = nn.Parameter(torch.zeros(config.head_size))
        
        if config.v_bias:
            if config.head_bias:
                self.v_bias = nn.Parameter(torch.zeros(config.n_head, config.head_size))
            else:
                self.v_bias = nn.Parameter(torch.zeros(config.head_size))
    
    def reset_parameters(self):
        self.attn.reset_parameters()
        self.proj.reset_parameters()
        if self.config.k_bias:
            torch.nn.init.zeros_(self.k_bias)
        
        if self.config.v_bias:
            torch.nn.init.zeros_(self.v_bias)
    ...
    def forward(
        ...
        ################################################
        if self.config.k_bias:
            q_bias = torch.zeros(B, 1, self.config.n_head, self.config.head_size, device=q.device, dtype=q.dtype)
            if self.config.v_bias:
                if self.config.head_bias:
                    k_bias = self.k_bias.repeat(B, 1, 1, 1).to(k.dtype)   # (B, 1, H, h_dim)
                    v_bias = self.v_bias.repeat(B, 1, 1, 1).to(v.dtype)
                else:
                    k_bias = self.k_bias.repeat(B, 1, self.config.n_head, 1).to(k.dtype)
                    v_bias = self.v_bias.repeat(B, 1, self.config.n_head, 1).to(v.dtype)
            else:
                if self.config.head_bias:
                    k_bias = self.k_bias.repeat(B, 1, 1, 1).to(k.dtype)   # (B, 1, H, h_dim)
                else:
                    k_bias = self.k_bias.repeat(B, 1, self.config.n_head, 1).to(k.dtype)
                v_bias = torch.zeros(B, 1, self.config.n_head, self.config.head_size, device=q.device, dtype=q.dtype)
            
            # print(f"v_bias norm: {v_bias.norm(p=2, dim=-1).mean()}")
            # v_bias = torch.zeros(B, 1, self.config.n_head, self.config.head_size, device=q.device, dtype=q.dtype)
            q = torch.cat([q_bias, q], dim=1)
            k = torch.cat([k_bias, k], dim=1)
            v = torch.cat([v_bias, v], dim=1)  # (B, T+1, H, h_dim)
            # print(f"v states norm: {v.norm(p=2, dim=-1).mean(dim=(0,2))}")
            
            y, attns = self.scaled_dot_product_attention(q, k, v, mask=mask, output_attention=output_attention)  # (B, T, H, C) 

            y = y[:, 1:, :, :]          # truncate the first token

        else:
            assert self.config.v_bias == True
            if self.config.head_bias:
                v_bias = self.v_bias.repeat(B, 1, 1, 1).to(v.dtype)
            else:
                v_bias = self.v_bias.repeat(B, 1, self.config.n_head, 1).to(v.dtype)  # (B, 1, H, C)
            y, attns = self.scaled_dot_product_attention(q, k, v, mask=mask, output_attention=output_attention) # (B, H, T, C) 
            y = y + v_bias
    ...
    def scaled_dot_product_attention(
        self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, mask: Optional[torch.Tensor] = None, output_attention: Optional[bool] = False,
    ):
        ...
        if output_attention:
            y, attn = eager_scaled_dot_product_attention(

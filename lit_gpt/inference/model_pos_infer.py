...
class GPTpos(nn.Module):
    ...
    def forward(
        ...
        if position_embeddings is not None:
            x = x + position_embeddings[:, :x.shape[1], :]
        ...
        if not use_kv_cache:
            for block in self.transformer.h:
                x, *_, attns = block(x, max_seq_length, output_attention=output_attention)
            ...
            for i, block in enumerate(self.transformer.h):
                x, self.kv_caches[i], attns = block(x,  max_seq_length, mask, input_pos, self.kv_caches[i], output_attention=output_attention)
                all_attns.append(attns)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        n_1 = self.norm_1(x)
        h, new_kv_cache, attns = self.attn(n_1, max_seq_length, mask, input_pos, kv_cache, output_attention=output_attention)

...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        y = self.scaled_dot_product_attention(q, k, v, mask=mask, prefix_token=self.config.prefix_token)
    ...
    def scaled_dot_product_attention(
        self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, mask: Optional[torch.Tensor] = None, prefix_token = 0,
    ):
        ...
        #     q, k, v, attn_mask=mask, dropout_p=self.config.attn_pdrop, scale=scale, is_causal=mask is None, prefix_token=prefix_token
        # )

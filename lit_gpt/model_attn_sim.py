    ...
    def forward(
        ...
        if not use_kv_cache:
            for block in self.transformer.h:
                x, *_ = block(x, (cos, sin), max_seq_length, attention_bias=self.attention_bias)
        else:
...
class Block(nn.Module):
    ...
    def forward(
        ...
        n_1 = self.norm_1(x)
        h, new_kv_cache = self.attn(n_1, rope, max_seq_length, attention_bias, input_pos, kv_cache)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        y = self.scaled_dot_product_attention(q, k, v, sim=self.sim, attention_normalize=self.attention_normalize, attention_bias=attention_bias)  # (B, H, T, C) 
    ...
    def scaled_dot_product_attention(
        self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, sim: str = 'sigmoid', attention_normalize: bool = True, attention_bias: Optional[torch.Tensor] = None
    ):
